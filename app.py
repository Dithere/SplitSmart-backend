...
@app.post("/expenses")
def add_expense(data: ExpenseCreate, user=Depends(get_current_user), db: Session = Depends(get_db)):
    expense = Expense(
    ...
    for uid in data.split_between:
        ...
        balance = db.query(Balance).filter_by(group_id=data.group_id, user_id=uid).first()
        if not balance:
            balance = Balance(group_id=data.group_id, user_id=uid)
            db.add(balance)
